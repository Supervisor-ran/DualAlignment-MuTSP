...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        df_data_prior = None
        #s--single TSP, m-multi TSP
        if self.features == 'M':
            # 这些列不参与数值特征
            drop_cols = ['date', 'start_date', 'end_date', 'prior_history_avg', text_name]
            base = df_raw.drop(columns=drop_cols, errors='ignore')


            # 只保留数值列，避免 StandardScaler 遇到字符串/类别报错
            num_cols = base.select_dtypes(include=[np.number]).columns
            #print('num_cols', num_cols)

            # 取数值特征作为 X
            df_data = df_raw.loc[:, num_cols]
        ...
        #only for Time-MMD
        if self.args.root_path.startswith("./data/"):
    ...
    def get_text_pair(self, indices):
        """
        返回两个二维字符串列表:
          hist_text_grid: [B, seq_len]，每个元素是 encoder 窗口对应时刻的文本
          fut_text_grid : [B, seq_len]，与上面一一对齐，取从 r_begin 开始的位置（未来锚点）
        这样两者的形状都与 batch_x 的 [B, seq_len] 匹配。
        """
        if isinstance(indices, torch.Tensor):
            indices = indices.numpy()

        B = len(indices)
        L = self.seq_len  # 与 batch_x 的第二维一致

        s_begins = indices % self.tot_len
        # s_end/r_begin/r_end 的定义与 __getitem__ 保持一致
        # s_end = s_begin + seq_len
        # r_begin = s_end - label_len
        # r_end   = r_begin + label_len + pred_len

        N = len(self.text)  # 文本序列总长度（与数据行数一致）

        def _safe_text_at(pos):
            if pos < 0 or pos >= N:
                return ""
            val = self.text[pos]
            # 兼容 df_raw[[text_name]] 带来的 (N,1) 或 list/ndarray 封装
        ...
        for s_begin in s_begins:
            ...
            for t in range(L):
                pos_hist = s_begin + t  # encoder 窗口内的第 t 个时间点
                pos_fut = r_begin + t  # 与之对齐的未来锚点（长度同 L）
                row_hist.append(_safe_text_at(pos_hist))
                row_fut.append(_safe_text_at(pos_fut))

            hist_grid.append(row_hist)
            fut_grid.append(row_fut)

        return hist_grid, fut_grid
